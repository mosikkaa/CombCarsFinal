...
const photoContainer = document.querySelector('.posteditemscontainer');
const leftArrow = document.getElementById('left-arrow');
const rightArrow = document.getElementById('right-arrow');

photoContainer.addEventListener('wheel', (e) =>{
  e.preventDefault();
  const scrollAmount = (e.deltaX !== 0 ? e.deltaX : e.deltaY)*5;
  photoContainer.scrollBy({ left: scrollAmount, behavior: 'smooth' });
},{ passive: false });

rightArrow.addEventListener('click', () =>{
  photoContainer.scrollBy({ left: photoContainer.clientWidth, behavior: 'smooth' });
})
